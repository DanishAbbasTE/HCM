...
import { Validation } from '../interfaces/validators/validations';
import { ValidatorDate } from '../interfaces/validators/date';
import { ValidatorParam } from '../interfaces/validators/param';
...
export class ValidatorService extends AngularServiceInjector {
  ...
  toTitleCase(str : string | any) {
    if (str) {
      const field = str.replace(/\w\S*/g,  (txt : string | any) => {
  ...
  handleRequired(control: FormControl) {
    let errObj : any = this._error_control(control);
    if (errObj) {
      if (errObj.key == 'ENTER') {
        errObj = 'Please enter ' + errObj.lbl;
      } else if (errObj.key == 'SELECT') {
        errObj= 'Please select ' + errObj.lbl;
      }
      return errObj;
    }
  }
  _error_server(httpErrorResponse: HttpErrorResponse) {
    ...
    server_response?.errors?.forEach((error: { detail: { message: any; }[]; field_name: any; }) => {
  ...
  _matchValidator(firstControl: any, secondControl: any, groupone: any, grouptwo: any) {
    return (group: FormGroup): Validation | null | any => {
...
export interface ServerValidationMessage {
  ...
  violation: string; // (Min / Max) Values => Maximum 100 characters allowed
}
